...
public class New_ProfAssimsgServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ResultSet r=db.Quer("SELECT idUsers_team FROM Users_team WHERE Team_idTeam='"+(String) request.getParameter("team")+"' AND Users_idUsers='"+(String) request.getParameter("jmbag")+"';");
		...
		if(db.Upd("INSERT INTO Messages SET Users_idUsers='"+Author+"', Users_team_idUsers_team='"+Wall+"', Subject='"+Subject+"', Content='"+Content+"', Date='"+Date+"', Time='"+Time+"';")){
    		...
    		try{
	    		...
	    		if(r.getString("Opt").charAt(3)=='0'){
		    		...
		    		r=db.Quer("SELECT Name FROM Team WHERE idTeam='"+(String) request.getParameter("team")+"';");
		    		r.first();
		...
		RequestDispatcher rd = sc.getRequestDispatcher("/ProfAssi_msg.jsp?ret="+ret);
